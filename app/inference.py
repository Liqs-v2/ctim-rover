...
SYSTEM_PROMPT = """You are a senior software engineer maintaining a large project.
...
def start_conversation_round_stratified(
    ...
    #   We need to make sure that the environment is reset, the agent will have tried to apply a patch already
    prompt = (
    ...
    # Reading Note: In our case the intent here will be `write_patch`
...
def start_conversation_round_state_machine(
    ...
    for round_no in range(start_round_no, globals.conv_round_limit + 1):
        ...
        # Reading NOTE: But then how does this work for non-GPT models if function_call_intents is empty? The order of the return values is also different
...
def run_one_task(
    ...
    Args:
        ...
        most_similar_task (str): The exemplar from SWE-Bench Lite with the most similar problem statement.
    """
    ...
    if globals.provide_exemplar:
        if most_similar_task is None:
            log.log_and_always_print(f"No exemplar available or no previous task was similiar enough (90% threshold) for {api_manager.task.task_id}.")
        else:
            formatted_past_trajectory = ""
            
            for entry in most_similar_task:
                formatted_past_trajectory += f"ROLE: {entry['role']}\nCONTENT: {entry['content']}\n\n"
                
            exemplar = ('Below follows an example of how you have previously solved a similar task. The example is delimited by the <start_example></end_example> tags. ' +
                        'IMPORTANT: The repository and details of your current task differ.\n\n' +
                        f'<start_example>{formatted_past_trajectory}\n</end_example>')
            
            log.log_and_always_print(f"Exemplar found for task {api_manager.task.task_id}. Passing exemplar.")
            msg_thread.add_user(exemplar)
    if (globals.provide_general_cross_episode_knowledge or globals.provide_repository_level_cross_episode_knowledge or globals.use_reflections) and combined_prompt is not None:
        log.log_and_always_print(f"Passing cross-episode knowledge.")
        msg_thread.add_user(combined_prompt)
